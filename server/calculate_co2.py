...
def main():
    ...
    if (redis.hget("total", "berlin").decode()):
...
def update_aircrafts_in_airspace(states, request_time):
    ...
    for state in states:
        ...
        if (longitude is None or latitude is None or velocity is None or true_track is None):
            continue
        ...
        if (aircrafts_in_airspace.get(state[0]) is not None):
            ...
            aircrafts_in_airspace[state[0]] = {"last_update": request_time, "positions": [position]}
...
# velocity in m/s
def calculate_duration(point1: tuple[float,float], point2: tuple[float,float], velocity: float):
    if (velocity == 0):
        return 0
...
def get_edge_position(true_track: float, 
    ...
    if (true_track >= 0 and true_track <= 90):  
        ...
        if (pos_lo + de >= lomax):
            # meets right edge
            edge_pos_la = pos_la + (math.tan(math.radians(90-true_track)) * (lomax - pos_lo))
            edge_pos_lo = lomax
        else:
            # meets top edge
            edge_pos_la = lamax
    ...
    if (true_track >= 90 and true_track <= 180):
        ...
        if (pos_la - de <= lamin):
            # meets bottom edge
            edge_pos_la = lamin
            edge_pos_lo = pos_lo + (math.tan(math.radians(180-true_track)) * (pos_la - lamin))
        else:
            # meets right edge
    ...
    if (true_track >= 180 and true_track <= 270):
        ...
        if (pos_la - de <= lamin):
            # meets bottom edge
            edge_pos_la = lamin
            edge_pos_lo = pos_lo - (math.tan(math.radians(90-(270-true_track))) * (pos_la - lamin))
        else:
            # meets left edge
            edge_pos_la = pos_la - de
            edge_pos_lo = lomin
    ...
    if (true_track >= 270 and true_track <= 360):
        ...
        if (pos_lo - de <= lomin):
            # meets left edge
            edge_pos_la = pos_la + (math.tan(math.radians(90-(360-true_track))) * (pos_lo - lomin))
            edge_pos_lo = lomin            
        else:
            # meets top edge
            edge_pos_la = lamax
